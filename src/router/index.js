...
const routes = [
...

  {
    path: '/order/:legacy(.*)',
    redirect: (to) => {
      // Extract everything after `/order/` and stop at first '&' if present
      const legacy = Array.isArray(to.params.legacy) ? to.params.legacy.join('/') : String(to.params.legacy || '');
      const tradeNo = legacy.split('&')[0];
      return { name: 'Payment', query: { trade_no: tradeNo } };
    },
    meta: {
      requiresAuth: false
    }
