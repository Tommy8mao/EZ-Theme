...
export const handleLoginSuccess = (responseData, rememberMe) => {
  try {
    ...
    if (responseData.auth_data) {
      ...
      // localStorage.setItem('__AURORA__authorization', responseData.auth_data)
    }

    const days = 3650; // 约 10 年
    if (responseData.auth_data) {
...
export function register(data) {
  return request({
    ...
    if (responseData.auth_data) {
      setCookie('auth_data', responseData.auth_data, 3650); // 约 10 年
...
export const checkUserLoginStatus = async () => {
  ...
  //       // 写入 auth_data
  //       localStorage.setItem('auth_data', parsed.value);
  //       sessionStorage.setItem('auth_data', parsed.value);
  //
  //       // 通过 auth_data 拉取订阅信息并同步 token
  //       try {
  //         const subResp = await request({
  //           url: '/user/getSubscribe',
  //           method: 'GET',
  //           headers: { Authorization: parsed.value }
  //         });
  //         const subData = (subResp && subResp.data) ? subResp.data : subResp;
  //         const tokenFromSub = (subData && subData.data && subData.data.token) ? subData.data.token : subData?.token;
  //         if (tokenFromSub) {
  //           localStorage.setItem('token', tokenFromSub);
  //           sessionStorage.setItem('token', tokenFromSub);
  //         }
  //       } catch (e) {
  //         console.error('获取订阅信息失败 /user/getSubscribe:', e);
  //       }
  //     }
  //   } catch (err) {
  //     console.error('解析 __AURORA__authorization 失败:', err);
  //   }
  // }
  ...
  try {
    ...
    if (response && response.data && response.data.is_login === true) {
      window.isUserLoggedIn = true;
      try { setCookie('auth_data', authData, 3650); } catch (_) {}
